...
- ✅ **Real sanctions screening** (`BP_SCREENING_PROVIDER=watchlist`): every outbound payment's counterparty is screened against the **UN Security Council Consolidated List** and the **US OFAC SDN list** (incl. alternate names), ingested by `backend/scripts/update-watchlists.mjs` with fail-closed validation and boot-time freshness gating; a weekly CI job re-runs the full pipeline against the live downloads ([`WATCHLISTS.md`](./WATCHLISTS.md)) — ⬜ remaining: commercial **PEP** data and tuned fuzzy matching (licensed screening vendor; no public PEP registry exists)
- 🟨 KYC provider registry in place (`src/kyc.js`, selected via `BP_KYC_PROVIDER`, fail-closed on unknown names, loud boot warning when production runs the sandbox) — ⬜ remaining: contract + integrate a licensed **KYC** vendor as a registry adapter
